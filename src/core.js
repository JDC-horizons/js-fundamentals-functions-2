...
function remainingBakeTime(mins) {
  return bakingTime() - mins
}
...
function calculatePreparationTime(layersNum) {
  const prepTime = layersNum * 3
  return prepTime
}
...
function totalTimeSpent(layersNum, mins) {
  const totalTime = calculatePreparationTime(layersNum) + mins
  return totalTime
}
